...
def map_cmu_to_smpl(motion: MotionData) -> MotionData | None:
    src_to_smpl: dict[int, int] = {}
    assigned_smpl: set[int] = set()

    for src_idx, name in enumerate(motion.joint_names):
        if name in CMU_TO_SMPL:
            smpl_idx = CMU_TO_SMPL[name]
...
def find_cmu_bvh_files(data_dir: Path) -> list[Path]:
